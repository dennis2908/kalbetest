...
use App\Mail\MailBarangLunas;
use App\Mail\PenjualMail;
   ...
   //         ->with('colors',$colorList);   
    ...
    public function search(Request $r){
        ...
        if(isset($category) && isset($name)){
            ...
            $sRes = DB::select( DB::raw("SELECT * FROM `products` WHERE lower(txtProductName) like '%$name%' and category_id = $category" ) );
            //dd("SELECT * FROM `products` WHERE lower(name) like '%$name%' and category_id = $category" );
            //$a = 0;
        }
        else if(isset($name)){
            $name = strtolower($name);
            $sRes = DB::select( DB::raw("SELECT * FROM `products` WHERE lower(txtProductName) like '%$name%'" ) );
	...
	public function auth_api(Request $r)
    {
		return $r->user();
	}	
    ...
    public function confirm_payment(Request $r)
    {   //Session::forget('cart');
        $res = Product::all();
        $cat = Category::all();
        $prov = Ongkir::all();
        if(!Session::has('cart'))
        {
            return view('store.confirm_payment')->with('all',null)
            ->with('products',[])
            ->with('products', $res)
            ->with('prov',$prov)
            ->with("cat", $cat);
    	...
    	return view('store.confirm_payment')
            ->with('products', $res)
            ->with("cat", $cat)
            ->with('all',$cart)
            ->with('prov',$prov)
    ...
    public function confirm_payment_post(Request $r)
    {   //Session::forget('cart');
        if($r->has('order'))
        {
            if(Session::has('user'))
            {
                ...
                $objBarangLunas = new \stdClass();
                $objBarangLunas->sender = 'Arifah Dilah';
                $objBarangLunas->receiver = session('user')->full_name;
                $objBarangLunas->cat = $cat; 
                $objBarangLunas->all = $cart; 
                $objBarangLunas->prod = $product; 
                $objBarangLunas->total = Session::get('price'); 
                $objBarangLunas->id_trans = "DMD-".session('user')->id.'-'.Session::get('id_sess'); 
        
                Mail::to(session('user')->email)->send(new MailBarangLunas($objBarangLunas));

                $objPenjual = new \stdClass();
                $objPenjual->sender = 'Arifah Dilah';
                $objPenjual->receiver = session('user')->full_name;
                $objPenjual->id_trans = "DMD-".session('user')->intCustomerID.'-'.Session::get('id_sess'); 
        
                Mail::to('dennismichaelmanullang@gmail.com')->send(new PenjualMail($objPenjual));
                
                $sales= new sale();

                $sales->where('id',Session::get('id_sess'))->update([
                    'user_id'=>session('user')->intCustomerID,
...
//    for quntity control in cart ENDS
    ...
    {  
        if($r->has('order'))
        {
            if(Session::has('user'))
            {
                ...
                $sales= new sale();
                $sales->user_id=session('user')->intCustomerID;
                $sales->product_id=session('cart');
                $sales->order_status='Menunggu Pembayaran';
                $sales->price=session('price');
                if(!Session::has('id_sess')){
                    $sales->save();
                    Session::put('id_sess',$sales->id);
                }
                else{
                   // dd(Session::get('id_sess'));
                    $sales->where('id',Session::get('id_sess'))->update([
                        'user_id'=>session('user')->intCustomerID,
           ...
           // dd(1);
            ...
            // Session::forget('orderCounter');
            //dd( $r->session());
            return redirect()->route('user.confirm_payment');
            }
            else{
                return redirect()->route('user.cart');
            }
    ...
    {
        $res1= sale::where('user_id', session('user')->intCustomerID)->get();
        if(!$res1)
        {
